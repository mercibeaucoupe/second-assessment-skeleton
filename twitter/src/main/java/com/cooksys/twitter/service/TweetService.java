...
public class TweetService {
	...
	@Transactional
	public ReplyDto reply(Integer id, ContentCredentials user, HttpServletResponse response) 
			throws IOException, TweetDoesNotExistException, CredentialsNoMatchException {
		...
		Reply replyTweet = new Reply(replier, new Timestamp(
				System.currentTimeMillis()).getTime(), user.getContent(), replyTo);
		replier.getTweets().add(replyTweet);
		// Add Method to parse tags

		return replyMapper.toDto(tweetRepository.save(replyTweet));
	}
	
    @Transactional
	public RepostDto repost(Integer id, CredentialsDto user, HttpServletResponse response) 
			throws IOException, TweetDoesNotExistException {
		...
		Repost repostTweet = new Repost(reposter, new Timestamp(System.currentTimeMillis()).getTime(), 
				repostTo);
		reposter.getTweets().add(repostTweet);
		
		return repostMapper.toDto(tweetRepository.save(repostTweet));
	}
	...
	@Transactional
	public SimpleTweetDto post(ContentCredentials contentCredentials, HttpServletResponse response) 
        ...
        SimpleTweet newTweet = new SimpleTweet(poster, contentCredentials.getContent(),
        		new Timestamp(System.currentTimeMillis()).getTime() );
        poster.getTweets().add(newTweet);
        
        return simpleTweetMapper.toDto(tweetRepository.save(newTweet));
